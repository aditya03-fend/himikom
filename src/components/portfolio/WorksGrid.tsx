...
export default function WorksGrid({ works }: { works: Work[] }) {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.from(cardsRef.current, {
        ...
        duration: 1,
        stagger: 0.1,
        ease: "back.out(1.5)", // Sedikit membal
        scrollTrigger: {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>, index: number) => {
    ...
    gsap.to(glow, {
      x: x,
      y: y,
      opacity: 0.6, // Nyalakan cahaya
      duration: 0.2, // Responsif cepat
      ease: "power2.out"
    });
    ...
    gsap.to(content, {
      x: (x - centerX) * 0.1, 
      y: (y - centerY) * 0.1,
      z: 50, // Angkat konten ke depan (TranslateZ)
      duration: 0.4,
      ease: "power2.out"
    });
  };
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 perspective-1000">
            {works.map((work, i) => (
                ...
                >
                    {/* 1. LIQUID NEON GLOW (Follower) */}
                    {/* Bola cahaya biru yang gerak ngikutin mouse */}
                    <div 
                    ...
                    {/* 2. TECH DECORATIONS (Corner Brackets) */}
                    {/* Siku-siku yang muncul pas hover */}
