...
export default function CreateWorkPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("https://aditya03-fend-himikom-backend.hf.space/api/work", {
        method: "POST",
        headers: {
      ...
      if (!res.ok) throw new Error("Gagal menyimpan karya");
    ...
    <div className="pb-20">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="lg:w-2/3 order-2 lg:order-1">
            ...
            <div className="bg-zinc-900 border border-white/10 rounded-xl overflow-hidden min-h-[400px] flex flex-col">
                <ReactQuill 
                    theme="snow"
        ...
        <div className="lg:w-1/3 order-1 lg:order-2 space-y-6">
           <div className="bg-zinc-900 border border-white/10 p-6 rounded-xl sticky top-32">
              ...
              <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex bg-black/50 p-1 rounded-lg border border-white/5">
                            <button 
                                ...
                                className={`p-1 px-2 rounded text-[10px] font-bold transition-all flex items-center gap-1 ${imageInputMode === 'file' ? 'bg-green-600 text-white' : 'text-gray-400 hover:text-white'}`}
                            >
                                <ImageIcon size={12} /> Upload
                            </button>
                            <button 
                                onClick={() => setImageInputMode('url')}
                                className={`p-1 px-2 rounded text-[10px] font-bold transition-all flex items-center gap-1 ${imageInputMode === 'url' ? 'bg-green-600 text-white' : 'text-gray-400 hover:text-white'}`}
                            >
                            ...
                            {imageInputMode === 'file' ? (
                                ...
                                <div 
                                    className={`relative w-full h-40 border-2 border-dashed rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all ${
                                        dragActive ? "border-green-500 bg-green-500/10" : "border-white/10 bg-black/30 hover:bg-black/50 hover:border-white/20"
                                    }`}
                ...
                {/* Info Tambahan (Static Text) */}
                <div className="p-4 bg-blue-900/20 border border-blue-500/20 rounded-lg mt-4">
                    <p className="text-xs text-blue-300 leading-relaxed">
                        Tips: Gunakan fitur <strong>Code Block</strong> di editor untuk menampilkan snippet kodingan agar terlihat lebih teknis dan keren.
                    </p>
                </div>
      ...
      <style jsx global>{`
        ...
        .ql-snow .ql-editor pre.ql-syntax {
            ...
            color: #22c55e !important; /* Matrix Green */
